...
export default defineConfig(({ mode, command }) => {
  ...
  const apiKey = env.TYPESAFE_API_KEY || env.AI_GATEWAY_API_KEY || '';
  const targetUrl = env.AI_GATEWAY_API_KEY && !env.TYPESAFE_API_KEY
    ? 'https://ai-gateway.vercel.sh/typesafe'
    : 'https://api.typesafe.ai';

  if (command === 'serve' && (!apiKey || apiKey.trim() === '')) {
    console.warn('⚠️ TYPESAFE_API_KEY or AI_GATEWAY_API_KEY is not set.');
  }
  return {
    server: {
      port: 5173,
