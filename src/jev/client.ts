...
export function createNodeJevClient(): TypeSafeClient {
  ...
  const defaultBaseURL = process.env['AI_GATEWAY_API_KEY'] && !process.env['TYPESAFE_API_KEY']
    ? 'https://ai-gateway.vercel.sh/typesafe'
    : 'https://api.typesafe.ai';
  const baseURL = process.env['TYPESAFE_BASE_URL'] ?? defaultBaseURL;
  return createJevClient({
    baseURL: baseURL.trim() === '' ? defaultBaseURL : baseURL,
    apiKey,
    allowBrowser: false,
  });
}
