  ...
  for (const cubie of cube.cubies) {
    for (const sticker of cubie.stickers) {
      if (cube.highlighted.has(sticker.facelet)) {
        // High-contrast glowing neon amber/gold spotlight for active target piece
        sticker.material.emissive.setHex(0xffaa00);
        sticker.material.emissiveIntensity = 0.85;
      } else if (cube.hovered === sticker.facelet) {
        sticker.material.emissive.copy(sticker.material.color).multiplyScalar(0.35);
        sticker.material.emissiveIntensity = 0.4;
      } else {
        sticker.material.emissive.setHex(0x000000);
        sticker.material.emissiveIntensity = 0.0;
      }
    }
  }
}
